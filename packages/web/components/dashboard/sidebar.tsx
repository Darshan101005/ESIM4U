...
export default function DashboardSidebar({ collapsed = false, onToggle }: DashboardSidebarProps) {
  ...
  const renderContent = (isCollapsed: boolean) => (
    <div className="flex flex-col h-full">
      <div className={`px-4 py-6 flex items-center gap-1.5 ${isCollapsed ? "justify-center px-2" : ""}`}>
        {onToggle && (
          <button
            onClick={onToggle}
        ...
        {isCollapsed ? (
          <div className="flex flex-col items-center gap-1">
            <Link
              ...
              className="w-9 h-9 rounded-xl bg-gradient-to-br from-[#FF561E] to-[#FF7A45] flex items-center justify-center shrink-0"
            >
