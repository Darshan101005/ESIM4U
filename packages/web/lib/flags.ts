...
export function toAlpha2(code: string | undefined): string {
  if (!code) return "";
  const upper = code.trim().toUpperCase();
  if (upper.length === 2) return upper;
  if (upper.length === 3) return ISO3_TO_ISO2[upper] || "";
  return "";
}

export function flagUrl(code: string | undefined, size: FlagSize = "m"): string {
  const alpha2 = toAlpha2(code);
  if (!alpha2) return "";
  return `/flags/${size}/${alpha2}.svg`;
}
